...
-- 4. write a query to find sub-categories where average profit is more than the half of the max profit in that sub-category ( 0 rows )
...
select sub_category , sum(quantity) as total_quantity_sold from orders
where region = 'West'
group by sub_category
...
select ship_mode  , region , sum(sales) as total_sales from orders
